...
>`select id, uid, cot_type, how, start, stale from cot_router order by start DESC limit 20;`

>`select id, uid, cot_type, time, start, round(ST_Y(event_pt)::numeric, 5) as latitude, round(ST_X(event_pt)::numeric, 5) as longitude from cot_router order by start desc li
mit 10;`
--- 
...
Connection
  \c[onnect] {[DBNAME|- USER|- HOST|- PORT|-] | conninfo}
                         connect to new database (currently "cot")
